...
namespace Jellyfin.Plugin.Curator.Core.Context
{
    ...
    /// When this was last drawn from. The only input to culling — a condition a
    /// place stopped producing (a snowy winter in a city that has since had three
    /// mild ones) ages out instead of sitting in the store forever.
    /// </param>
    ...
    /// <summary>
    /// Picking a row title for the conditions, and keeping the store of them from
    /// growing forever.
    ///
    /// <para>
    ...
    /// five titles bought for it the first time serve every rainy cold evening
    /// afterwards for nothing. A place produces on the order of thirty distinct
    /// conditions, so the cost curve flattens within weeks and then stays flat —
    /// against one call per refresh forever, which is what generating a fresh title
    /// each time would mean.
    /// </para>
    ///
    /// <para>
    ...
    /// to a rainy evening in November, and two viewers in the same weather do not
    /// see the same words.
    /// </para>
    /// </summary>
    public static class ContextTitles
    {
        ...
        /// a condition the place genuinely stopped producing, or one left behind by
        /// a vocabulary change.
        /// </remarks>
        ...
        /// the model does not start reaching — asked for twenty variations on "it is
        /// raining", the last ten are noticeably worse than the first five.
        /// </remarks>
        public const int DefaultTitlesPerCondition = 5;

        /// <summary>
        /// The cache key for one set of conditions.
        /// </summary>
        /// <remarks>
        /// Words are sorted, so <c>rain, cold</c> and <c>cold, rain</c> are one
        /// condition rather than two — the order Open-Meteo happens to report them in
        /// is not a fact about the weather, and treating it as one would double the
        /// number of conditions bought for no gain.
        /// <para>
        /// The daypart row keys on the daypart alone and the weather row on the
        /// weather alone. Combining them would multiply the conditions eightfold to
        /// describe rows that only ever claim one of the two.
        /// </para>
        /// </remarks>
        /// <param name="kind">Which row.</param>
        /// <param name="context">The conditions.</param>
        /// <returns>A stable key.</returns>
        public static string ConditionKey(ContextRowKind kind, ViewingContext context)
        {
            ArgumentNullException.ThrowIfNull(context);

            if (kind == ContextRowKind.Daypart)
            {
                return "daypart:" + ContextVocabulary.WordFor(context.Daypart);
            }
            ...
            return "weather:" + string.Join(',', words);
        }
        ...
        /// </summary>
        /// <remarks>
        /// The other half of culling, and the one that is not about time: a key
        /// naming a word the vocabulary no longer has can never be matched again, so
        /// it is dead the moment the vocabulary changes rather than a year later. An
        /// empty weather key is dead too — a weather row is not drawn without a
        /// reading, so nothing would ever ask for it.
        /// </remarks>
        ...
        public static bool IsLiveCondition(string? condition)
        {
            ...
            var body = condition[(separator + 1)..];
            ...
            return kind switch
            {
                "daypart" => words.Length == 1 && ContextVocabulary.IsDaypart(words[0]),
                "weather" => words.All(ContextVocabulary.IsWeather),
                _ => false,
            };
        }
        ...
        /// </summary>
        /// <remarks>
        /// The viewer's own offset is mixed into the position so two people in the
        /// same weather do not read the same words on the same evening — which is
        /// what would otherwise happen the moment per-viewer rows exist, since they
        /// share a condition and therefore a set.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// Two reasons to drop one, and they are different in kind. A set naming a
        /// word the vocabulary has lost is dead immediately — nothing can ever match
        /// it again. A set that merely has not been drawn from is dropped only after
        /// a long time, because these conditions are seasonal and an eager rule would
        /// re-buy every winter what it culled every summer.
        /// <para>
        ...
        /// cannot be culled out from under the row that is using them.
        /// </para>
        /// </remarks>
        ...
        /// <summary>
        /// The conditions written out for a model to read.
        /// </summary>
        /// <param name="kind">Which row.</param>
        /// <param name="context">The conditions.</param>
        /// <returns>Something like <c>a cold, rainy evening</c>.</returns>
        public static string Describe(ContextRowKind kind, ViewingContext context)
        {
            ArgumentNullException.ThrowIfNull(context);
            ...
            var weather = context.Weather.Count == 0
                ? "unremarkable weather"
                : string.Join(" and ", context.Weather.Select(WeatherPhrase));

            return string.Create(CultureInfo.InvariantCulture, $"{weather}");
        }
